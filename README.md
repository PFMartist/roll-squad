...
- 干员池是**全游戏 429 名可获取干员**，对应 **2026-09-04「石白深蓝之夜」** 那次更新
  （女神异闻录3 Reload 联动，该批新增结城理 / 岳羽由加莉 / 埃癸斯 / 虎狼丸；解包主表快照抓取于 2026-09-22）——
  页面页脚、BOX 下拉里都写着这个版本，练度是按名字哈希洒开的合成数据；
- **不是你账号的干员池**，也不写任何记录；**不含干员立绘**，卡片走职业色块 + 名字首字（在线版不放任何角色形象）；
- 交互全是真的：开始编队、锁定 / 单格重抽、六个档位、星级、配额、职业竖栏都能点。

要抽自己的干员池，用下面的桌面版。

> 演示池由 `app/make_demo_roster.py` 从解包主表生成。游戏出新干员后跑一遍、提交，
> Pages 会自动重新部署；脚本顶部的版本常量（`GAME_VERSION` / `NEWEST_BATCH` 等）要一并更新
> —— 注意**联动干员的 char_ID 是提前分配的**，别拿「ID 最大」当最新。
...
`syncTime` 超过 14 天，BOX 徽章会变黄提醒你重新同步（练度会变）。
...
- **档位 2「精一满级」与档位 4「模组线」是按星级算的**，不是一刀切：
  精一满级 = 各星级的精英一上限（3★55 / 4★60 / 5★70 / 6★80）；
  模组线 = 该星级开模组的门槛等级（4★40 / 5★50 / 6★60）。
  另外精英化会重置等级（有人 E2 却不到精一上限），所以 **E2 的干员一律算过「精一满级」**。
- **阿米娅**是全游戏唯一能转职的干员（术师/近卫/医疗）。配额凑不够时她会被顶上去，卡片上标「转职」。
- **新干员不用管**：职业索引里查不到的会联网现查一次，还是查不到就记「未知」，照常能抽，不会被剔除。
- **头像取过就留下**：缓存在 `data\avatars\`，同一张图只下一次；之后断网也有脸。
- **抽签本身从不联网**。
...
前端就是 `web\`：浏览器打开 `web\index.html?demo=1` 能看内置演示（16 名干员、不联网），
